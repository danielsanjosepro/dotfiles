...
local function patch_vim_treesitter_query()
  local ok, err = pcall(vim.treesitter.query.get, 'vim', 'highlights')
  if ok or not tostring(err):find('Invalid node type "tab"', 1, true) then
    return
  end

  local query_path = vim.fn.stdpath('data') .. '/lazy/nvim-treesitter/runtime/queries/vim/highlights.scm'
...
vim.api.nvim_create_user_command('TypstPin', function()
  local client = vim.iter(vim.lsp.get_clients()):find(function(lsp_client)
    return lsp_client.name == 'tinymist'
  end)
  ...
  client:request('workspace/executeCommand', {
    ...
    if err then
      vim.notify('error pinning: ' .. vim.inspect(err), vim.log.levels.ERROR)
    else
